 ...
 ***********************************************************************/
matrix * do_add( matrix * A, matrix * B, int subtract )
{
  if( !A || !B )
  {
...
matrix * fill_val( matrix * a, double val )
{
  if( ! a )
    return 0;
...
matrix * get_column( matrix * A, int column_to_get )
{
...
matrix * get_row( matrix * A, int row_to_get )
{
...
void swap_rows( matrix * a, int x, int y )
{
...
matrix * lu( matrix * a, int verbose )
{
  ...
  for( int x = 0; x < (n-1); x++ )
  {
    ...
    {
      //printf("Comparing for swap %f < %f\n", fabs(cur_pivot), fabs(A->values[row][x]) );
    ...
    for( int row = x + 1; row < n; row++ )
    {
      ...
      {
        A->values[ row ][ column ] = A->values[ row ][ column ] - ( A->values[ row ][ x ] * A->values[ x ][ column ]);
      }
    }
    if( verbose >= 2 )
    {
...
matrix * qr( matrix * A )
{
  ...
  for( int i = 1; i < A->columns; i++ )
  {
    ...
    for( int j = 0; j < i; j++ )
    {
      ...
      //v = add( v, multiply( multiply( transpose( v ), get_column( Q, j ) ), get_column( Q, j ) ), 1 );
